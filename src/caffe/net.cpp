...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  LOG(INFO)<<"split_diff = "<<split_diff*sizeof(Dtype);
  if(phase()==TRAIN){
	  to_cpu(param);
	  LOG(INFO)<<"workspace_size_before_share = "<<Caffe::workspace_size_before_share();
	  LOG(INFO)<<"workspace_size_in_bytes_of_cudnn_conv_ = "<<Caffe::workspace_size_of_cudnn_conv();
	  LOG(INFO)<<"intermediate data size = "<<get_intermediate_data_size();
  }
}
...
//		DLOG(INFO)<<"memory block "<<i<<" ref: "<<memory_blocks[i].getRef();
		// -1 means this memory_block is unoccupied(avaliable), we can assign it to top_blob's data
...
template <typename Dtype>
void Net<Dtype>::to_cpu(const NetParameter &param){
...
//		DLOG(INFO) << i << blob_names_[i] << " reference: " << *(blobs_[i]->ref());
//		DLOG(INFO) << i <<blob_names_[i] << " reference: " <<blobs_ref_[i];
//	}
	for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
		/*for(int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id)
			DLOG(INFO) << "\tlayer " <<layer_id<<" "<< layer_names_[layer_id] <<", top blob "<< blob_names_[top_id_vecs_[layer_id][top_id]]
			<<" reference: " << *(top_vecs_[layer_id][top_id]->ref());*/
		const LayerParameter& layer_param = param.layer(layer_id);

		if(layer_param.type() == "BatchNorm"){
			recompute_layer_ids_.insert(layer_id);
...
//			for(int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id){
//				DLOG(INFO)<<"layer_id: " << layer_id << ", " << layer_names_[layer_id]
...
//			}
			...
			/*for(int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id){
				DLOG(INFO)<<"layer_id: " << layer_id << ", " << layer_names_[layer_id]<< ", top blob "<<blob_names_[top_id_vecs_[layer_id][top_id]]
...
//		DLOG(INFO)<<i<<", "<<memory_blocks[i].getMemptr()->size()<<", SyncedMem: "<<memory_blocks[i].getMemptr();
	...
	for(int i = 1; i < split_layer_ids_.size(); ++i){// ignore label_split layer
...
//	DLOG(INFO)<<"allocated syncedmem size = "<<allocated_syncedmem_.size();
	...
	vector<bool> state_of_memblock;// the size of state_of_memblock equals to the number of memblocks we need.
	for(int i = layers_.size()-1; i > 0; --i){
		if(inplace_layer_ids_.find(i) == inplace_layer_ids_.end()){
			for(int j = 0; j < bottom_vecs_[i].size(); ++j){
				int number_of_memblock = bottom_vecs_[i][j]->getMemBlockNumOfDiff();
				CHECK_EQ(number_of_memblock, -1);
				vector<bool>::iterator iter = std::find(state_of_memblock.begin(),
						state_of_memblock.end(), false);
				if(iter != state_of_memblock.end()){
//					DLOG(INFO)<<"iter = "<<(iter - state_of_memblock.begin());
					bottom_vecs_[i][j]->setMemBlockNumOfDiff(iter - state_of_memblock.begin());
					state_of_memblock[iter - state_of_memblock.begin()] = true;//true means its state is occupied.
				}else{//when all memblocks are occupied, we need expand state_of_memblock
					bottom_vecs_[i][j]->setMemBlockNumOfDiff(state_of_memblock.size());
					state_of_memblock.push_back(true);
				}
				/*DLOG(INFO)<<"layer id: "<<i<<", blob "<<j<<" name: "<<blob_names_[bottom_id_vecs_[i][j]]<<
						", diff id = "<<bottom_vecs_[i][j]->getMemBlockNumOfDiff();*/
			}
			if( loss_layer_ids_.find(i) == loss_layer_ids_.end()){
				for(int j = 0; j < top_vecs_[i].size(); ++j){
					int number_of_memblock = top_vecs_[i][j]->getMemBlockNumOfDiff();
					CHECK_NE(number_of_memblock, -1);
					CHECK_LT(number_of_memblock, state_of_memblock.size());
					// set the state of memory block to unoccupied, false means its state is unoccupied
					state_of_memblock[number_of_memblock] = false;
				}
			}
		}
	}
	vector<shared_ptr<SyncedMemory> > shared_diffs;
	for(int i = 0; i < state_of_memblock.size(); ++i){
		shared_ptr<SyncedMemory> tmp(new SyncedMemory(1));
		shared_diffs.push_back(tmp);
	}
	for(int i = 0; i < blobs_.size(); ++i){
		int number_of_memblock = blobs_[i]->getMemBlockNumOfDiff();
		if(number_of_memblock != -1){
			shared_diffs[number_of_memblock]->resize(blobs_[i]->count() * sizeof(Dtype));
			blobs_[i]->SetDiffTo(shared_diffs[number_of_memblock]);
			/*DLOG(INFO)<<"diff of blob["<<i<<"] "<<blob_names_[i]<<"= "<<number_of_memblock<<
					", size = "<<blobs_[i]->diff()->size();*/
		}
	}
}
...
int Net<Dtype>::AppendBottom(const NetParameter& param, const int layer_id,
  ...
  if(layer_param.type() == "Split"){
//	  DLOG(INFO)<<"Split layer "<<layer_param.name()<<" bottom blob " << blob_names_[blob_id] << " count: "<<layer_param.top_size();
...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  /*Timer forward_timer;
  Timer timer;
  std::vector<double> forward_time_per_layer(layers_.size(), 0.0);
...
//    DLOG(INFO) << std::setfill(' ') << std::setw(25) << layer_names_[i] <<
//          "\tforward: " << forward_time_per_layer[i] / 1000  << " ms.";
  }
//  forward_time += forward_timer.MicroSeconds();
//  DLOG(INFO) << "Forward pass: " << forward_time / 1000 << " ms.";
  return loss;
}
...
void Net<Dtype>::BackwardFromTo(int start, int end) {
  ...
  Timer timer;
  std::vector<double> backward_time_per_layer(layers_.size(), 0.0);
...
//  DLOG(INFO) << "Backward pass: " << backward_time / 1000 << " ms.";
}
...
//	ostringstream transfered_layers;
	for (int i = start; i <= end; ++i) {
	    for (int c = 0; c < before_forward_.size(); ++c) {
	      before_forward_[c]->run(i);
	    }
...
//		CUDA_CHECK(cudaStreamSynchronize(0));
		// after a layer's calculation finished, it's bottom data will transfer to cpu,
		// noted: only layer in 'the_rest_laye_ids_' can be transfered,
		// like relu/dropout/.. these layer's bottom data does not need transfer
		if (i > 0 && i < end - 2
...
//				transfered_layers << i << " ";
				if(*(bottom_vecs_[i][j]->ref()) == 1){
					...
					bottom_vecs_[i][j]->TransferToCPU(stream);
				}
			}
		}
		Dtype layer_loss = layers_[i]->Forward(bottom_vecs_[i], top_vecs_[i]);
...
//			SLOG(INFO)<<"\t"<<blob_names_[bottom_id_vecs_[i][j]]<<" reference: " << *(bottom_vecs_[i][j]->Ref());;
		}
...
//		DLOG(INFO) << std::setfill(' ') << std::setw(30) << layer_names_[i] <<
//        	"\tforward: " << forward_time_per_layer[i] / 1000  << " ms.";
	}
//	forward_time += forward_timer.MicroSeconds();
//	DLOG(INFO) << "Forward pass: " << forward_time / 1000 << " ms.";
//	DLOG(INFO) << transfered_layers.str();
	return loss;
}
...
//				<< layer_names_[i] /*<< ", bottom blob " << blob_names_[bottom_id_vecs_[i][0]]*/;
		if (layer_need_backward_[i]) {
//			DLOG(INFO)<<"layer id = "<<i<<", "<<layer_names_[i];
//			timer.Start();
			layers_[i]->Backward(top_vecs_[i], bottom_need_backward_[i], bottom_vecs_[i]);
			if (debug_info_) { BackwardDebugInfo(i); }
		}
		if (i > 2 && i < start && the_rest_layer_ids_.find(i - 2) != the_rest_layer_ids_.end()) {
			/*for(int j = 0; j < top_vecs_[i].size(); ++j){
			...
			for(int j = 0; j < bottom_vecs_[i - 2].size(); ++j){
				if(*(bottom_vecs_[i - 2][j]->ref()) == 0){
					...
					for(int k = 0; k < top_vecs_[i].size(); ++k)
						if(bottom_vecs_[i - 2][j]->data() == top_vecs_[i][k]->data())
							CUDA_CHECK(cudaStreamSynchronize(0));
					bottom_vecs_[i - 2][j]->TransferToGPU(stream);
	//				transfered_layers << i - 2  << " ";
					bottom_vecs_[i - 2][j]->decreaseRef();
				}
			}
		}
		if(recompute_layer_ids_.find(i-2) != recompute_layer_ids_.end()){
			layers_[i-2]->RegainTemp(bottom_vecs_[i-2]);
		}
		CUDA_CHECK(cudaStreamSynchronize(stream));
		CUDA_CHECK(cudaStreamSynchronize(0));
		for (int c = 0; c < after_backward_.size(); ++c) {
