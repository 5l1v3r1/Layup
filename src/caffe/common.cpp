...
Caffe::Caffe()
    : cublas_handle_(NULL), curand_generator_(NULL),
	  #ifdef USE_CUDNN
	  cudnn_handle_(NULL),
	  workspace_size_in_bytes_of_cudnn_conv_(0),
	  workspace_size_before_share_(0),
	  workspaceData_(NULL),
	  tempDataSize_(0),
	  tempData1_(NULL),
	  tempData2_(NULL),
...
void Caffe::SetDevice(const int device_id) {
  ...
  Get().workspace_size_in_bytes_of_cudnn_conv_ = 0;
  Get().workspace_size_before_share_ = 0;
  Get().workspaceData_ = NULL;
  Get().tempDataSize_ = 0;
  Get().tempData1_ = NULL;
  Get().tempData2_ = NULL;
#endif
