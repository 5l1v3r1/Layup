...
void CuDNNLCNLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (totalSizeInBytes > Caffe::tempDataSize()) {
//    tempDataSize = totalSizeInBytes;
    Caffe::set_tempDataSize(totalSizeInBytes);

    cudaFree(Caffe::tempData1());
    cudaFree(Caffe::tempData2());

    // allocate new buffers
    CUDA_CHECK(cudaMalloc(&Caffe::tempData1(), totalSizeInBytes));
    CUDA_CHECK(cudaMalloc(&Caffe::tempData2(), totalSizeInBytes));
  }
}
...
//  cudnnDestroy(handle_);
  ...
  // free temp buffers
  CUDA_CHECK(cudaFree(Caffe::tempData1()));
  CUDA_CHECK(cudaFree(Caffe::tempData2()));
}
