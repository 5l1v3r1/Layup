...
//	CHECK(!data_cpu_ptr_);
	data_cpu_ptr_ = data_->transfer_to_cpu(stream, sizeof(Dtype) * count(),
			data_cpu_ptr_);
//	DLOG(INFO)<<"data_cpu_ptr: "<<data_cpu_ptr_;
}
...
//		DLOG(INFO)<<"data cpu_ptr: "<<data_cpu_ptr_<<", data gpu_ptr: "<<data_->gpu_data();
	}
}
...
Blob<Dtype>::~Blob() {
#ifndef CPU_ONLY
	if (Caffe::mode() == Caffe::GPU && data_cpu_ptr_ != NULL) {
		DLOG(INFO)<<"~Blob()...";
